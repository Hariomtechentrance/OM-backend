...
// Products data - simplified for sequential import
const productsData = [
...
async function addProductsSequentially() {
  try {
    console.log('Starting sequential product import...');
    ...
    for (let i = 0; i < productsData.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          ...
          totalErrors++;
          console.error(`❌ Error creating product ${product.name}:`, result.message);
        }
