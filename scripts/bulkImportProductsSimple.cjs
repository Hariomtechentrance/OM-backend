...
// Products data - simplified version
const productsData = [
...
async function bulkImportProducts() {
  try {
    ...
    const winterCollection = await Collection.findOne({ slug: 'winter-collection' });
    const monsoonCollection = await Collection.findOne({ slug: 'monsoon-collection' });

    if (!winterCollection || !monsoonCollection) {
      console.error('❌ Collections not found');
      return;
    }

    console.log(`✅ Found Winter Collection: ${winterCollection.name}`);
    console.log(`✅ Found Monsoon Collection: ${monsoonCollection.name}`);
      ...
      try {
        const createdProducts = await Promise.all(
          batch.map(async (productData) => {
            ...
            if (productData.collectionSlug === 'winter-collection') {
              collectionId = winterCollection._id;
            } else if (productData.collectionSlug === 'monsoon-collection') {
              collectionId = monsoonCollection._id;
            }
        ...
        console.log(`✅ Batch ${Math.floor(i/batchSize) + 1}: Created ${createdProducts.length} products`);
